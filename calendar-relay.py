...
# used to validate the credentials sent by Google
app.config["google_access_token"] = ""
...
def authenticate():
    """
    This function compares the credentials being sent by Google to our expected
    values.
    ...
    if expected_client_id != args.get("client_id") or expected_client_secret != args.get(
        "client_secret"
    ):
        logging.info("Wrong oauth credentials.")
        return dict()
    return dict(access_token=app.config["google_access_token"])  # this is a fake token
...
def parse_google_request(request):
    ...
    """
    expected_access_token = app.config["google_access_token"]
    # Verify access token in the request
    try:
        if expected_access_token != request.headers["Authorization"].split(" ")[1]:
            logging.info("Wrong oauth credentials.")
            abort(401)
    except:
        abort(400)
    ...
    timing["availability_view_interval"] = body["ns2:FreeBusyViewOptions"][
        "ns2:MergedFreeBusyIntervalInMinutes"
    ]
    accounts = []
